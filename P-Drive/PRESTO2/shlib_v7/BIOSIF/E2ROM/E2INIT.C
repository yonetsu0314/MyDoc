/*----------------------------------------------------------------------*
 * File name	: e2init.c	/ initialize e2rom context		*
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/biosif/e2rom/RCS/e2init.c 1.3 1970/01/01 00:00:00 chimura Exp $
 *$Log: e2init.c $
...
//	imask_bak = get_imask();
//	set_imask(15);
	err = (int)trapa_svc(VECTOR_E2ROM, 0x0, contxt, time);
//	set_imask( imask_bak );
	return(err);
}
