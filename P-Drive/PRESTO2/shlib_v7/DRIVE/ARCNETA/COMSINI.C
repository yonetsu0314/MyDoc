 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/arcneta/RCS/comsini.c 1.2 1970/01/01 00:00:00 sakaguti Exp $
 *$Log: comsini.c $
 * リビジョン 1.2  1970/01/01  00:00:00  sakaguti
 * 2004/07/21 15:30:00
 * arcnet_sys_initで、ARCNET ICが無い場合の初期化エラーのときは
 * arc_exist_flag[]、pc->iobaseを０にし、正常リターンするように変更。
 * 初期化正常時は、arc_exist_flag[]を１にセットし、正常リターンする。
 * 
...
/*	ARCNET IC の初期化を行う。		*/
/*	成功した場合					*/
/*			arc_exist_flag[devno] = 1;		*/
/*			pc->iobase = phy_inf->io_adr;	*/
/*	エラーの場合					*/
/*			arc_exist_flag[devno] = 0;		*/
/*			pc->iobase = 0;			*/
/*	となる。						*/
/*----------------------------------------------------------------------*/
/* 引き数								*/
/*	devno 	: channel number					*/
/*	*prmp  : pointer of parameter list				*/
/*	rx_tsk 	: Receive Task for wup_tsk						*/
/*									*/
/* 戻り値								*/
/*	int	:  0	･･･ success				*/
/*		  -1	･･･ error					*/
