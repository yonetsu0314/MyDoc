 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/arcneta/RCS/commap.c 1.4 1970/01/01 00:00:00 sakaguti Exp $
 *$Log: commap.c $
 * リビジョン 1.4  1970/01/01  00:00:00  sakaguti
 * 2004/07/21 15:30:00
 * 各種ＭＡＰ関数node_hunt,arc_nextid,arc_map,get_arc_map,arc_contact
 * について、ARCNET IC が無い場合、検出ﾉｰﾄﾞなしの応答をするようにした。
 * 
	...
	)
{
	...
	if((pc->iobase)==0) return(0);	/* ICが無い場合ﾉｰﾄﾞなしとする */
	...
	)
{
	...
	if((pc->iobase)==0) return(-1);	/* ICが無い場合ﾉｰﾄﾞなしとする */
	...
	)
{
	...
	if(arc_exist_flag[devno]==0) return(-1);	/* ICが無い場合ﾉｰﾄﾞなしとする */
	ph = &com_hd_tbl[devno];
...
{
	if(arc_exist_flag[devno]==0) return((char *)0);	/* ICが無い場合ﾉｰﾄﾞなしとする */
	...
	)
{
	...
	if(arc_exist_flag[devno]==0) return(0);	/* ICが無い場合ﾉｰﾄﾞなしとする */
	pc = &com_ctxt_tbl[devno];
