 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/pdadc/RCS/dadfzt.c 1.3 1970/01/01 00:00:00 kawamura Exp $
 *$Log: dadfzt.c $
 ...
 * 
 *----------------------------------------------------------------------*
 *Header: p:/presto/shlib_v7/drive/ndadc/rcs/dadfzt.c 1.4 1970/01/01
 *Log: dadfzt.c
	...
	)
{
	...
	*zero_ad = rpkt.zero_ad[ch_no];
	if(rpkt.err_ptn.wei_bp)					return(1);
	return(0);
}
	...
	)
{
	...
	err = auto_zero(pwb,ch_no,zero_ad,ZERO_AD_ADJ);
	return( err );
}

int
dad_az3rd(					/* stack = 76byte	*/
	DAD_ID		pwb,
	int		ch_no,
	short		*zero_ad
	)
{
	...
	err = auto_zero(pwb,ch_no,zero_ad,ZERO_AD_F3RD);
	return( err );
}

int
dad_az5th(					/* stack = 76byte	*/
	DAD_ID		pwb,
	int		ch_no,
	short		*zero_ad
	)
{
	...
	err = auto_zero(pwb,ch_no,zero_ad,ZERO_AD_F5TH);
	return( err );
}
