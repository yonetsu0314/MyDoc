 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/pdadc/RCS/dadmzt.c 1.2 1970/01/01 00:00:00 chimura Exp $
 *$Log: dadmzt.c $
 ...
 * 
 *----------------------------------------------------------------------*
 *Header: p:/presto/shlib_v7/drive/ndadc/rcs/dadmzt.c 1.4 1970/01/01
 *Log: dadmzt.c
	...
	)
{
	...
	if(buf.ptn.wei_bp == 0 && buf.ptn.df_bp == 0 && buf.ptn.afv_bp == 0) return(0);
...
#endif	/* MAS_BIG_ENDIAN */
	err_ptn->wei_bp = ret.err_ptn.wei_bp;
	err_ptn->df_bp  = ret.err_ptn.df_bp;
	err_ptn->afv_bp = ret.err_ptn.afv_bp;
	for(i =0; ptn.wei_bp ; ptn.wei_bp >>=0x1L, ret.err_ptn.wei_bp >>=0x1L, i++) {
		if(ptn.wei_bp & 0x1) {
			if(ret.err_ptn.wei_bp & 0x1) {
	...
	bp  = ptn.df_bp;
	ebp = ret.err_ptn.df_bp;
	for(i =0; bp ; bp >>=0x1L, ebp >>=0x1L, i++) {
		if(bp & 0x1) {
			if(ebp & 0x1) {
				err_sts[i+DAD_WEI_MAX] = ret.zero_ad[i+DAD_WEI_MAX];
			}
			else {
				zero_ad[i+DAD_WEI_MAX] = ret.zero_ad[i+DAD_WEI_MAX];
				zero_da[i+DAD_WEI_MAX] = ret.zero_da[i+DAD_WEI_MAX];
			}
		}
	}
	bp  = ptn.afv_bp;
	ebp = ret.err_ptn.afv_bp;
	for(i =0; bp ; bp >>=0x1L, ebp >>=0x1L, i++) {
		if(bp & 0x1) {
			if(ebp & 0x1) {
				err_sts[i+DAD_WEI_MAX+DAD_DF_MAX] = ret.zero_ad[i+DAD_WEI_MAX+DAD_DF_MAX];
			}
			else {
				zero_ad[i+DAD_WEI_MAX+DAD_DF_MAX] = ret.zero_ad[i+DAD_WEI_MAX+DAD_DF_MAX];
				zero_da[i+DAD_WEI_MAX+DAD_DF_MAX] = ret.zero_da[i+DAD_WEI_MAX+DAD_DF_MAX];
			}
		}
	}
	if(err_ptn->wei_bp || err_ptn->df_bp || err_ptn->afv_bp)	return(1);
	return(0);
}
