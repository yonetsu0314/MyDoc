 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/pdadc/RCS/dadbot2.c 1.2 1970/01/01 00:00:00 chimura Exp $
 *$Log: dadbot2.c $
 ...
 * 
 *----------------------------------------------------------------------*
 *Header: p:/presto/shlib_v7/drive/ndadc/rcs/dadbot2.c 1.4 1970/01/01
 *Log: dadbot2.c
...
struct txt {					/* text format		*/
	...
	DAD_ARY_PTN	whptn;			/* 指定号機ﾋﾞｯﾄ･ﾊﾟﾀｰﾝ	*/
	DAD_ARY_PTN	dfptn;			/* 指定号機ﾋﾞｯﾄ･ﾊﾟﾀｰﾝ	*/
	DAD_ARY_PTN	afvptn;			/* 指定号機ﾋﾞｯﾄ･ﾊﾟﾀｰﾝ	*/
	...
	)
{
	...
	buf.whptn.wei_bp  = arg->ptn.wei_bp;
	buf.whptn.df_bp   = 0;
	buf.whptn.afv_bp  = 0;
	buf.dfptn.wei_bp  = 0;
	buf.dfptn.df_bp   = arg->ptn.df_bp;
	buf.dfptn.afv_bp  = 0;
	buf.afvptn.wei_bp = 0;
	buf.afvptn.df_bp  = 0;
	buf.afvptn.afv_bp = arg->ptn.afv_bp;
...
int	ielm1;
	...
	CHG_UL_ULONG(text->whptn.wei_bp);
	CHG_UL_SHORT(text->whptn.df_bp);
	CHG_UL_SHORT(text->whptn.afv_bp);
	CHG_UL_ULONG(text->dfptn.wei_bp);
	CHG_UL_SHORT(text->dfptn.df_bp);
	CHG_UL_SHORT(text->dfptn.afv_bp);
	CHG_UL_ULONG(text->afvptn.wei_bp);
	CHG_UL_SHORT(text->afvptn.df_bp);
	CHG_UL_SHORT(text->afvptn.afv_bp);
