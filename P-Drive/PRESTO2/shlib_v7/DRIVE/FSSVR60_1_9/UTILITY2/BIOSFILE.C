 ...
 * Function 	:
 *----------------------------------------------------------------------
 * $Header: p:/presto2/shlib_v7/drive/rcs/biosfile.c 1.1 1970/01/01 00:00:00 kagatume Exp $
 * $Log: biosfile.c $
...
int SramFormat( void )
{
	...
	ClearBackupFlg(SRAM_DRIVE);

	/* バックアップ領域のクリア */
	ClearBackupArea(SRAM_DRIVE);
...
int CfFormat( void )
{
	...
	ClearBackupFlg(CF_DRIVE);

	/* バックアップ領域のクリア */
	ClearBackupArea(CF_DRIVE);
