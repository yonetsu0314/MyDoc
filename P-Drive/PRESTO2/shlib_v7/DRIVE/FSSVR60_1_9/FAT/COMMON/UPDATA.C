 ...
 * Original     :
 *----------------------------------------------------------------------
 *$Header: p:/presto2/shlib_v7/drive/fssvr60/fat/common/rcs/updata.c 1.4 1970/01/01 00:00:00 kagatume Exp $
 *$Log: updata.c $
...
/*       セクタ数/クラスタを比較して決定する                                  */
/* 01.08 更新終了位置がセクタ境界に位置する場合、更新終了位置を－１して       */
/*       更新終了位置のセクタ番号を求める                06/01/16 Y.Yoshizumi */
/*       更新終了位置以降のクラスタ内残りセクタをコピーする際、               */
/*       更新終了位置がセクタ境界に位置する場合、コピー先セクタを-1する       */
...
)
{
	...
	F_BYTE l_sectflg;                   /* セクタ境界フラグ ≠0：セクタ境界   */
	...
	*( newtopclstnum ) = 0;
	...
	else
	{
		...
		/************************************************/
		l_work04 = datasize;
		if ( (l_work01 & l_modbps) == 0 )
		{
			/* セクタ境界に書込み終了位置が位置する場合、１つ前にする。 */
			/* (FMR_setPointerでは、セクタ境界の場合、次のセクタに位置させる為  */
			l_work04--;
			l_sectflg = 1;
		}
