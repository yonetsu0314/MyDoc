 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/drive/scu/sh7615/sci/RCS/sciint.c 1.3 1970/01/01 00:00:00 chimura Exp $
 *$Log: sciint.c $
...
sci1_rxint_top(){
	...
	if(wup_flg[SCIF_CH1]) wup_tsk((ID)scif_rx_tsk[SCIF_CH1]->acadr);
...
sci2_rxint_top(){
	...
	if(wup_flg[SCIF_CH2]) wup_tsk((ID)scif_rx_tsk[SCIF_CH2]->acadr);
