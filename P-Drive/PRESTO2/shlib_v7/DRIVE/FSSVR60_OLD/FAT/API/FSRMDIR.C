 ...
 * Original     :
 *----------------------------------------------------------------------
 *$Header: p:/presto2/shlib_v7/drive/fssvr60/fat/api/RCS/fsrmdir.c 1.1 1970/01/01 00:00:00 kagatume Exp $
 *$Log: fsrmdir.c $
...
/* <040811> ファイル・ディレクトリ存在チェック修正    2004.08.11 T.Tsuneyoshi */
/******************************************************************************/
...
)
{
	...
	/* 検索ハンドルの確認を行う                                               */
	/**************************************************************************/
	for( l_work02 = 0; l_work02 < DFMR_USR_MAXSEARCHHANDLE; l_work02 ++ )
	{
		if( ( l_targetsect ==  tfmr_searchhandle[ l_work02 ].sectornum )
			&& ( l_targetpos ==  tfmr_searchhandle[ l_work02 ].position )
			&& ( l_drvnum == tfmr_searchhandle[ l_work02 ].drivenum ) )
		{
			/* ディレクトリエントリのセクタ/セクタ内位置が、検索ハンドルの    */
			/* 同パラメータと等しい場合は対象ファイルが検索中と見なす         */
			/* 対象ファイルを初期化する */
			FMR_initSearchHandle(
				( F_INT )l_work02       /* ループカウンタを                   */
				                        /* 検索ハンドル番号とする             */
				);
		}
	}
