 ...
 * Original     :
 *----------------------------------------------------------------------
 *$Header: p:/presto2/shlib_v7/drive/fssvr60/fat/api/RCS/fsremove.c 1.2 1970/01/01 00:00:00 kagatume Exp $
 *$Log: fsremove.c $
...
)
{
	...
	/* 検索ハンドルの確認を行う                                               */
	/**************************************************************************/
	for( l_work01 = 0; l_work01 < DFMR_USR_MAXSEARCHHANDLE; l_work01 ++ )
	{
		if( ( l_targetsect ==  tfmr_searchhandle[ l_work01 ].sectornum )
			&& ( l_targetpos ==  tfmr_searchhandle[ l_work01 ].position )
			&& ( l_drvnum == tfmr_searchhandle[ l_work01 ].drivenum ) )
		{
			/* ディレクトリエントリのセクタ/セクタ内位置が、検索ハンドルの    */
			/* 同パラメータと等しい場合は対象ファイルが検索中と見なす         */
			/* 対象ファイルを初期化する */
			FMR_initSearchHandle(
				( F_INT )l_work01       /* ループカウンタを                   */
				                        /* 検索ハンドル番号とする             */
				);
		}
	}
