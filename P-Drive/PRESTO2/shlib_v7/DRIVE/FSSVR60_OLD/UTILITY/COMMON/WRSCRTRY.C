/*======================================================================
 * File name    : wrscrtry.c
 * Original     :
 *----------------------------------------------------------------------
 *$Header: p:/presto2/shlib_v7/drive/fssvr60/utility/common/RCS/wrscrtry.c 1.2 1970/01/01 00:00:00 kagatume Exp $
 *$Log: wrscrtry.c $
...
)
{
	...
	/* パラメータ設定 */
	l_drvioparam.write.startsctnum = sectnum;   /* 書込み開始セクタ番号       */
	l_targetsctnum = 1;
	l_drvioparam.write.writesctnum = &l_targetsctnum;   /* 書込みセクタ数     */
	l_drvioparam.write.writebuff        /* 書込みデータ格納領域               */
	...
	for( l_retrycnt = 0; l_retrycnt < DFMR_USR_SDRETRY; l_retrycnt ++ )
	{
		l_drtn = MFMR_DRVENTRY( drvnum )(
			...
			DFMR_DRVFUNCWRITE,          /* Func.番号=Write                    */
			&l_drvioparam               /* I/Oパラメータ                      */
			);
		if( l_targetsctnum != 1 )       /* 書込みセクタ数が1以外の場合は      */
		{                               /* オーバフローを起こしたと見なす     */
