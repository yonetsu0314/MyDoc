 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/mon/presto/RCS/mrt010.c 1.1 1970/01/01 00:00:00 chimura Exp $
 *$Log: mrt010.c $
 * リビジョン 1.1  1970/01/01  00:00:00  chimura
 * 初期リビジョン
 * 
 *----------------------------------------------------------------------*
 *Header: p:/presto/shlib_v7/mon/src/rcs/mrt010.c 1.1 1970/01/01
 *Log: mrt010.c
 *----------------------------------------------------------------------*/
	...
	)
{
	...
	if((i = atol(argv[5])) <= 0) {
		mon_text_snd( ILLEGAL_PRM4, (void **)argl);	return(-1);
	}
	boot_pk.wh_limit = (short)i;
	if((i = atol(argv[6])) <= 0) {
		mon_text_snd( ILLEGAL_PRM5, (void **)argl);	return(-1);
	}
	boot_pk.afv_limit = (short)i;
	if((i = atol(argv[7])) <= 0) {
		mon_text_snd( ILLEGAL_PRM6, (void **)argl);	return(-1);
	}
	boot_pk.afv_onoff_limit = (short)i;
