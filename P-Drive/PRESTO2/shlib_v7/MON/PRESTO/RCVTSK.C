 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/mon/presto/RCS/rcvtsk.c 1.1 1970/01/01 00:00:00 chimura Exp $
 *$Log: rcvtsk.c $
 * リビジョン 1.1  1970/01/01  00:00:00  chimura
 * 初期リビジョン
 * 
 *----------------------------------------------------------------------*
 *Header: p:/presto/shlib_v7/mon/src/rcs/rcvtsk.c 1.2 1970/01/01
 *Log: rcvtsk.c
 *----------------------------------------------------------------------*/
	...
	)
{
	...
	while(1) {
		err = SLP_TSK();
		assert(! err);
		len = (int)con_gets(&buf[0], (int)sizeof(buf));
		if(len  && (len != COM_FAIL)) {
			if(len == RCV_ERROR)
