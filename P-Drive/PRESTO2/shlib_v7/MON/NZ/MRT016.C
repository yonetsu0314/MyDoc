/*----------------------------------------------------------------------*
 * File name	: mrt016.c	/ monitor text scan			*
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/mon/nz/RCS/mrt016.c 1.1 1970/01/01 00:00:00 chimura Exp $
 *$Log: mrt016.c $
 * リビジョン 1.1  1970/01/01  00:00:00  chimura
 * 初期リビジョン
 * 
 *----------------------------------------------------------------------*
 *Header: p:/wcu/main/nz_wcu/uti/rcs/mrt016.c 1.1 1996/07/22 19:24:48
 *Log: mrt016.c
 *----------------------------------------------------------------------*/
...
err_retp3:		mon_text_snd( ILLEGAL_PRM3, argl);	return(-1);
	...
	if((len = dad_dsp_memwr( pwbid, (char *)mem, &load[0], byte) ) <= 0) {
		if(len == 0)
			return( mon_text_snd( DSP_WRITE_UNEXE, argl) );
		else {
			mon_text_snd( DSP_WRITE_FAIL, argl );	return(-1);
		}
	}
	return( mon_text_snd( DSP_WRITE_SUCES, argl) );
}
