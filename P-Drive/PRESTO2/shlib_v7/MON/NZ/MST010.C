/*----------------------------------------------------------------------*
 * File name	: mst010.c	/ set monitor text			*
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/shlib_v7/mon/nz/RCS/mst010.c 1.1 1970/01/01 00:00:00 chimura Exp $
 *$Log: mst010.c $
 * リビジョン 1.1  1970/01/01  00:00:00  chimura
 * 初期リビジョン
 * 
 *----------------------------------------------------------------------*
 *Header: p:/wcu/main/nz_wcu/uti/rcs/mst010.c 1.1 1996/07/22 19:25:57
 *Log: mst010.c
 *----------------------------------------------------------------------*/
	...
	)
{
	...
	*(cp++) = (char)'\r';
	p = argl[1];
	i = 8;
	for(m =0 ; m < DAD_WEI_MAX ; m++) {
		if(i == 8)  *(cp++) = (char)'\t';
		ltoh(*(unsigned long *)p, cp);
		p += sizeof(long);
		cp += sizeof(long)*2;
		if(--i) *(cp++) = (char)' ';
		else {
