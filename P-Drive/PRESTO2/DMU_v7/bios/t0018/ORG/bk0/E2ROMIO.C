 ...
 *----------------------------------------------------------------------*
 *$Header: p:/presto2/dmu_v7/bios/src/rcs/e2romio.c 1.3 1970/01/01 00:00:00 kagatume Exp $
 *$Log: e2romio.c $
...
int e2rom_pgwr(struct e2ioh  *drvinfop){
	union uci		shrt;
	unsigned char	*bp;
	short	gofs,pofs,eofs,bufmax,*flashadr;
	int		retry = 5;

//	rsstr("*** e2rom pgwr ***");
start:
	if((--retry)==0) goto err;
	eofs = *(short *)(drvinfop->filep);
	bp   = (unsigned char *)drvinfop->e2buf;
	gofs = *(short *)(drvinfop->getp);
	pofs = *(short *)(drvinfop->putp);
	bufmax = (short)drvinfop->szbuf;
