 ...
 *		初期設定ルーチン（FOR SH7615）
 *----------------------------------------------------------------------
 *$Header: p:/presto2/dmu_v7/bootload/rcs/init.c 4.1 1970/01/01 00:00:00 kagatume Exp $
 *$Log: init.c $
...
int aply_start_check(void)
{
	...
	/* dipswがONの場合はブートローダーへ */
	if(tmp = (FORCE_DOWNLOAD_ON)) {
		/* ブートライターを書き換えた場合、ブートライター起動 */
		if(memcmp((const void *)BOOT_WRITE_STATUS,(const void *)"BTWrite",(size_t)7)==0){
			return 0;
		}
		/* ブートローダーを書き換えた場合、アプリ復元を行う */
		if(memcmp((const void *)BOOT_WRITE_STATUS,(const void *)"ResmApl",(size_t)7)==0){
			if(comGYfunc()!=0) return -1;
		}
		wcu_turn_bootloader();
		return -1;
	}

	/* WDTによるリセットがかかった場合はブートローダーへ */
	if ( (tmp =((*RSTCSRR) & 0x0080)) != 0 ) {
		send_wcu_boot_cmd();
		return -1;
	}
	
	/* CFカードがある場合、電源投入後はDMUのCFチェックを行う */
	if (PCCard_check()==E_OK) {
		wcu_turn_bootloader();	/* comGBfuncにおいてバージョンチェック等、WCUとの通信を行うため */
		if(comGBfunc()!=0) {
			set_ledmode(FUNC_ERROR);
			return -1;
		}
		return 0;
	}
...
#else
	/* ブートライター起動時、ブートローダー書き換え処理を行う */
	rsstr("\nBootLoader for BootLoader on DMU/SH2");
	comBTwrite3();
	return -1;
#endif
}
...
#ifdef BTWRITER
	/* ブートライター時の割り込みテーブル設定 */
	for (i = 0; i < 128; i++) *((UW *)INTVEC+i) = *((UW *)BIOSVEC+i);
	ap = (struct apl_info *)(*((APLY*)BOOT_APLY_ADDR));
	vec = ap->vecter_adr;
	for(i = *(int *)ap->vect_no; i; i--) {
		*((V_FUNC *)INTVEC+i+RL1_P) = *vec++;
