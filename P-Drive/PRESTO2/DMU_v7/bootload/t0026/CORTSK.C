 ...
 * 		CON通信 受信タスク
 *----------------------------------------------------------------------
 *$Header: p:/presto2/dmu_v7/bootload/rcs/cortsk.c 2.1 1970/01/01 00:00:00 kagatume Exp $
 *$Log: cortsk.c $
...
{
	int_buf0_in  = int_buf0;
	int_buf0_out = int_buf0;
	int_buf0_count= (unsigned int)0;
	int_buf0_cntout= (unsigned int)0;
	int_buf0_ovfl = (unsigned char)0;
}

/*****************************************************************************
*
*	関数名: void con1_buf_clear(void)
*	機　能: シリアル受信パラメータクリア	
*
*	引き数:	なし
*
...
{
	int_buf1_in  = int_buf1;
	int_buf1_out = int_buf1;
	int_buf1_count= (unsigned int)0;
	int_buf1_cntout= (unsigned int)0;
...
{
	return(((int_buf0_count-int_buf0_cntout)<(INT_BUF_size/1024))?1:0);
}
...
{
	return(((int_buf1_count-int_buf1_cntout)<(INT_BUF_size/1024))?1:0);
}
