 ...
 * Function 	: 
 *----------------------------------------------------------------------
 *$Header: p:/presto2/dmu_v7/bootload/rcs/bootsubs.c 3.1 1970/01/01 00:00:00 kagatume Exp $
 *$Log: bootsubs.c $
...
filebkup(char fname[],int n){
	int		rdhdl,wrhdl,i,size;
	char	c,filename[80];
	
	strcpy(filename,CFDIRNAME);	strcat(filename,fname);
	rdhdl = IFS_fopen(filename, "r");
	if( rdhdl < 0 )	return (61);	/* File for Backup did not exist in CF */

	strcpy(filename,DMUBKDIR);	strcat(filename,"\\");	strcat(filename,fname);
	wrhdl = IFS_fopen(filename, "w");
	if( wrhdl < 0 )	return (62);	/* Can not Create File in CF */

	for(i=0;;){
		if((size=IFS_read(rdhdl,rdwrbuf,RDWR_SIZE))<=0) break;
		if(IFS_write(wrhdl,(void *)rdwrbuf,(unsigned int)size)<0) goto err;
		if(rschk()){
			c = rsin();
			if(c==(char)0x1b){
				IFS_fclose(rdhdl);
...
rmbkall(){
	...
	if( ret ) return(0);			/* /BOOTBAK not exist */

	for(i=0;i<16;++i) bkbuf[i][0] = (char)0;
	for(i=0;i<16;){
		if (i==0)	ret = findfirst("*", 0xff, &findinfo);
		else		ret = findnext(&findinfo);
		if( ret < 0 )	break;
		a = (int)findinfo.attrib;
		if(a & 0x08){
			rsstr("\n#");
			rsstr((char *)&findinfo.name);	rsout((char)'#');
			put8hex((int)findinfo.size);
			continue;
		}
		else if(a & 0x10){
			rsstr("\n<");
			rsstr((char *)&findinfo.name);
			rsout((char)'>');
			continue;
		}
		else{
			...
			rsstr((char *)&findinfo.name);	rsout((char)' ');
			put8hex((int)findinfo.size);
			strcpy(bkbuf[i],(char *)&findinfo.name);
			++i;
			continue;
		}
	}
	for(i=0;i<16;++i){
		...
		ret = IFS_remove( (char*)buf );
		if( ret ) rsstr(" remove file error.");
	}
	IFS_rmdir( (char*)DMUBKDIR );
...
loadINF(char infname[]){
	static char	buf[120],c;
	unsigned short	sum,sumsub;
	int		rdhdl,i,j;

	rdhdl = IFS_fopen(infname, "r");
	if( rdhdl<0 ) return(-1);
...
recvINF(){
	...
	for(;;){
		...
		if(tcpinput == 0){
			...
			if(IFS_write(wrhdl,(void *)crlf,(unsigned int)strlen(crlf))<0) goto err;
		}
	...
	if(tcpinput == 0) IFS_fclose(wrhdl);
	Bin_Asc16( (long)sum, (unsigned char *)(&buf[0]), (size_t)4 );
	buf[4] = (char)0;
	rsstr("\nsum = ");
	rsstr(buf);
...
upldcfasc(char fname[],int barcnt){
	...
	for(k=0;;){
		size = IFS_read(rdhdl,rdwrbuf,RDWR_SIZE);
		if(size<=0) {
			rsout((char)0x1a);		/* EOF */
			IFS_fclose(rdhdl);
			rsstr("\nRCU>OK\n");
