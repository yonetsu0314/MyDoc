 ...
 *----------------------------------------------------------------------
 *$Header: p:/presto2/dmu_v7/bootload/rcs/bootcmd1.c 4.1 1970/01/01 00:00:00 kagatume Exp $
 *$Log: bootcmd1.c $
...
int comGDfunc(void)
{
	...
	if(tcpinput == 0){
		...
		if (prgname[1][0] != '\0') {
			...
			rsstr("\nRCU>DISP 3:Backup "); rsstr(buf); rsstr(". (DMU-CF)\n");
...
next0:
	...
	if(recvINF()<0){			/* SUM CHECK */
		return recover_program(1);	/* Receive INF Error */
	}
	else{
		rsstr("\nRCU>OK\n");
	}
	...
	dly_tsk(5000);	/* CF内部書込み遅延がある場合に備えてウェイトを設ける */
...
static int comGU_FPGA_progcheck(void)
{
	...
	if(strcmp(prgname[1],tmpprogname)!=0) goto error;
	if(strcmp(prgver[1],tmpversion)!=0) goto error;
	if(strcmp(prgsum[1],tmpsumnum)!=0) goto error;
	rsstr("No Difference.");
	return 0;
...
static int comGU_WCU_progcheck(void)
{
	...
	if(strcmp(prgname[2],tmpprogname)!=0) goto error;
	if(strcmp(prgver[2],tmpversion)!=0) goto error;
	if(strcmp(prgsum[2],tmpsumnum)!=0) goto error;
	rsstr("No Difference.");
	return 0;
...
static int comGU_DMU_progcheck(void)
{
	...
	comN();			/* BOOT_VERSION_ADDR,versionstr */
	pickversion(versionstr);
	if(strcmp(prgname[3],tmpprogname)!=0) goto error;
	if(strcmp(prgver[3],tmpversion)!=0) goto error;
	if(strcmp(prgsum[3],tmpsumnum)!=0) goto error;
	rsstr("No Difference.");
	return 0;
...
int comGBfunc(void)
{
	...
	if(tcpinput == 0){
		...
		strcpy(buf,DMUBKDIR); strcat(buf,"\\"); strcat(buf,BKNOW);
		rdhdl = IFS_fopen(buf, "r");
		if( rdhdl>=0 ){		/* バックアップ途中で中断 */
			IFS_close(rdhdl);
			rsstr("\nBROKEN File exist\n");		
			rmbkall();
			goto next0;
