...
*  All rights reserved.                                                       *
*                                                                             *
*  Compile : shc -cp=sh3 -op=1 -def=CLK=? nos7709.c         (Hitachi C)       *
*            gcc -c -m3 -O2 -DCLK=? nos7709.c               (exeGCC)          *
*            ccsh -c -cpu=sh_iii -ansi -O -DCLK=? nos7709.c (GHS)             *
*                                                                             *
* 98-09-07 作成                                                               *
* 22/Apr/2000 NORTi Version4 用として                                         *
* 28/Sep/2000 CH0 選択時の不具合修正                                          *
* 31/Oct/2000 IAR EW 対応                                                     *
...
チャネル番号の指定
          ...
          (例) shc <options> nos7709.c  ･･･････････････････････ チャネル0
               copy nos7709.c nos7ch1.c
               shc <options> -def=CH=1 nos7ch1.c  ･････････････ チャネル1

          (例) gcc <options> nos7709.c  ･･･････････････････････ チャネル0
               gcc <options> -DCH=1 -onos7ch1.o nos7709.c ･････ チャネル1

          (例) ccsh <options> nos7709.c  ･･････････････････････ チャネル0
               ccsh <options> -DCH=1 -o nos7ch1.o nos7709.c ･･･ チャネル1

ボーレートジェネレータ

          ソースクロックのデフォルトは 20 MHz です。変更する場合 は CLK マク
          ロを定義してコンパイルしてください。値は [Hz] 単位の周波数です。

          (例) shc <options> -def=CLK=30000000 nos7709.c ･･････ 30 MHz
                                  ~~~~~~~~~~~~
          (例) gcc <options> -DCLK=30000000 nos7709.c
                               ~~~~~~~~~~~~
送受信バッファサイズの指定
...
割込み優先順位
          ...
          (例) shc <options> -def=IP=5 nos7709.c ･･･ 割込み優先順位 5
                                  ~~~~
          (例) gcc <options> -DIP=5 nos7709.c
                               ~~~~
未サポート機能
...
#define MHz         1000000L
#ifndef CLK
#define CLK         20*MHz      /* 周辺モジュール用クロック[Hz] */
#endif
...
static int get_stat(void)
{
  ...
  #else
    ssr = sfr_in(SCI_SSR) & (SSR_BRK|SSR_FER|SSR_PER|SSR_RDF);
...
static BOOL init_sio(UB mode, UW baud)
{
  ...
  #ifdef CH2
    if (SIO[CH].flag & TSF_RTS)
        sfr_set(SCIF_SCFCR, 0x80);      /* MCE = 1: モデム信号を許可 */
  #endif
...
L1:
  ...
  #else
    sts = sfr_in(SCI_SSR) & (SSR_BRK|SSR_FER|SSR_PER|SSR_RDF);
  #endif
  ...
  #else
    sfr_out(SCI_SSR, (UB)~(SSR_ER|SSR_BRK|SSR_FER|SSR_PER|SSR_RDF|SSR_DR));
  #endif
  ...
  #ifndef CH0
    if (sfr_in(SCI_SSR) & (SSR_ER|SSR_BRK|SSR_RDF|SSR_DR))
        goto L1;
  #endif
}
