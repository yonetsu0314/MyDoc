...
リモートデバッガ自動判別
          ...
          ROMでの実行か、ルネサスモニタプログラム上での実行の自動判別には、ルネサス
          モニタプログラムバージョンに依存しています。サンプルでは、Ver 3.2set
          で動作が確認されています。それ以外のバージョンをお使いの場合には
          カスタマイズが必要です。カスタマイズには、本ソースにバージョン依存行
          を追加し、コンパイルオプションで HITACH_MON_VER にバージョン番号
          を定義してください。

          バージョン依存行には、ルネサスモニタプログラムのベクタ先頭から１６個分
          (アドレス=0x00～0x3F)のベクタアドレスを使用しています。この値は
          ルネサスモニタプログラムのメモリダンプコマンド("MD 0")の実行によって、
          取得してください。

          ルネサスモニタプログラム Ver 4.1 に対応する場合には、
...
#ifndef HITACH_MON_VER
#define HITACH_MON_VER 0x0302
#endif

#if HITACH_MON_VER == 0x0302
/* ルネサスモニタプログラム(SH7615-DSP Self Debugger Ver 3.2set)
 * ベクタテーブル定義値(一部)
 */
static const UW hitach_mon_vec[] = {
    0x00004644L,0x08000000L,0x0000466AL,0x08000000L,
    0x0000292AL,0x00000000L,0x000046A4L,0x00000000L,
    0x00000000L,0x0000468CL,0x000046BCL,0x0000430CL,
    0x0000430CL,0x00000000L,0x00000000L,0x00000000L
};
#endif

static BOOL hitach_monitor;
...
/*****************************************************************************
* ルネサスモニタプログラム上で実行しているか否かをチェックする
*
* 形式  is_hitachi_monotir();
*
* 解説  ルネサスモニタプログラムで実行しているかを判定します。
******************************************************************************/

BOOL is_hitachi_monotir(void)
{
    return hitach_monitor;
}
...
{   /* 割込みテーブルをROM上に定義した場合は INTVSZ == 0 */
    ...
    if (p == (int *)0L)
      ...
      #ifdef HITACH_MON_VER

        /* ルネサスモニタプログラムの場合 */

        p = (int *)0L;
        for (i = 0; i < (sizeof(hitach_mon_vec)/sizeof(UW)); i++)
            if (*((UW *)p+i) != hitach_mon_vec[i])
                break;
        if (i == (sizeof(hitach_mon_vec)/sizeof(UW))){
            for (i = 0; i <= 105; i++)  /* copy vector for MONITOR-PG */
                if (*((UW *)p+i) != 0L)
                    *((UW *)INTVEC+i) = *((UW *)p+i);
            set_vbr((void **)INTVEC);
            hitach_monitor = 1;
