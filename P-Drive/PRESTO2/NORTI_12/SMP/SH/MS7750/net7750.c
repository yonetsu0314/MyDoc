...
#include "nosio.h"
  ...
  #ifdef MSWCEDB02
  const char title[] = "\r\n*** NORTi TCP/IP Sample Program for MS7750SE01 + MSWCEDB02 ***";
  #else
  const char title[] = "\r\n*** NORTi TCP/IP Sample Program for MS7750SE01 ***";
  #endif
#include "nos7750.h"
#include "sh7750.h"
...
TASK MainTask(void)
{
    ...
    /* (注) EDO-DRAM の後半 1MB (8C100000～8C1FFFFF) を使用します.  */
    /* この領域に変数やスタックを割り当てないよう注意してください. */
    ercd = disk_ini(disk, (char *)"A:", ramdisk, 0x8c100000, 0x100000);
    if (ercd != E_OK)
        goto END;
