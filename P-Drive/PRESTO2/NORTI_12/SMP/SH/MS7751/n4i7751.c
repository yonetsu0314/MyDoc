/****************************************************************************
* インターバル割込みハンドラ（MS7751SE01)                                   *
*                                                                           *
*  File name : n4i7751.c                                                    *
*  Copyright (c)  1995-2002, MiSPO Co., Ltd.                                *
*  All rights reserved.                                                     *
*                                                                           *
*  Compile : shc -cp=sh4 -op=1 -def=CLK=?,CH=? n4i7751.c        (Renesas C) *
*            gcc -c -m4 -O2 -DCLK=? -DCH=? n4i7751.c              (exeGCC)  *
*            ccsh -c -ansi -noasmwarn -cpu=sh4 -O                           *
*                                        -DCLK=? -DCH=? n4i7751.c (GHS)     *
*                                                                           *
* 24/Apr/2000                                                               *
* 12/Sep/2000 カウンタ値の計算を修正                                        *
* 05/Seb/2001 v4_def_inhで、パラメータがNULL, NADRの時の処理を修正          *
* 31/Oct/2002 Tick read function for HOOK definition updated          [SP]  *
...
使用するハードウェア
          ...
          SH7751 内蔵 TMU を周期タイマとして使用しています。
...
割込み周期の指定
          ...
          (例) shc <options> -def=MSEC=5 n4i7751.c
                                  ~~~~~~
          (例) gcc <options> -DMSEC=5 n4i7751.c
                               ~~~~~~
クロックの指定
          ...
          (例) shc <options> -def=CLK=27000000 n4i7751.c ... 19.6608 MHz
                                  ~~~~~~~~~~~~
          (例) gcc <options> -DCLK=27000000 n4i7751.c
                               ~~~~~~~~~~~~
チャネル番号の指定
          ...
          (例) shc <options> -def=CH=1 n4i7751.c  .......... TMU CH1
                                  ~~~~
          (例) gcc <options> -DCH=2 n4i7751.c  ............. TMU CH2
                               ~~~~
割り込みハンドラへの分岐

          割り込みの入口から例外コードを判別して、本ファイルの割り込みハンド
          ラへ分岐（jmp）するまでのルーチンは、ユーザにてアセンブラで記述して
          ください。（例が vec7751?.asm にあります）
...
割込み優先順位
          ...
          (例) shc <options> -def=IP=2 n4i7751.c ... 割込み優先順位 2
                                  ~~~~
          (例) gcc <options> -DIP=2 n4i7751.c
                               ~~~~
dis_int, ena_int
...
#define MHz         1000000L
#ifndef CLK
#define CLK         (27*MHz) /* 周辺クロックPφ[Hz] */
#endif

/* 割込みベクタアドレス */

#ifndef INTVEC
#define INTVEC      0x8c000000L
#endif

/* 割り込みハンドラテーブル */

UW intdef[INT_TUNI4-INT_NMI+1][2];
...
ER intini(void)
{
    ...
    p = &RST_VECT;
    if (p == (int *)0xac000000)     /* previous 0xa000000 */
        set_vbr((void **)INTVEC);
    else
    {
      #ifdef REBOOT
