...
* シルアル入出力ドライバ（SH7729R）                                         *
*                                                                           *
*  File name : nos7729R.c                                                   *
*  Copyright (c)  2003, MiSPO Co., Ltd.                                     *
*  All rights reserved.                                                     *
*                                                                           *
*  Compile : shc -cp=sh3 -op=1 -def=CLK=? nos7729R.c          (Renesas C)   *
*            gcc -c -m3 -O2 -DCLK=? nos7729R.c                (exeGCC)      *
*            ccsh -c -cpu=sh_iii -ansi -O -DCLK=? nos7729R.c  (GHS)         *
*                                                                           *
* 07/Sep/1998 作成                                                          *
* 22/Apr/2000 SH7709A 用として                                              *
* 28/Sep/2000 CH0 選択時の不具合修正                                        *
* 31/Oct/2000 IAR EW 対応                                                   *
* 20/Jun/2001 SH7729R 用として                                              *
...
チャネル番号の指定
          ...
          (例) shc <options> nos7729R.c  ----------------------- チャネル0
               copy nos7729R.c nos7ch1.c
               shc <options> -def=CH=1 nos7ch1.c  -------------- チャネル1

          (例) gcc <options> nos7729R.c  ----------------------- チャネル0
               gcc <options> -DCH=1 -onos7ch1.o nos7729R.c  ---- チャネル1

          (例) ccsh <options> nos7729R.c  ---------------------- チャネル0
               ccsh <options> -DCH=1 -o nos7ch1.o nos7729R.c  -- チャネル1
...
ボーレートジェネレータ
          ...
          (例) shc <options> -def=CLK=25000000 nos7729R.c  ------ 25 MHz
                                  ~~~~~~~~~~~~
          (例) gcc <options> -DCLK=25000000 nos7729R.c
                               ~~~~~~~~~~~~
送受信バッファサイズの指定
...
割込み優先順位
          ...
          (例) shc <options> -def=IP=5 nos7729R.c  --- 割込み優先順位 5
                                  ~~~~
          (例) gcc <options> -DIP=5 nos7729R.c
                               ~~~~
未サポート機能
...
static int get_stat(void)
{
  ...
  #else
    ssr = sfr_in(SCI_SSR) & (SSR_BRK|SSR_FER|SSR_PER|SSR_RDF|SSR_TEND);
