/*---------------------------------------------------------------------
 * $Header: p:/presto/shlib_v7/drive/usnets/usnetsh7/netsrc/RCS/ftp.c 1.2 1970/01/01 00:00:00 masahiro Exp $
 * $Log: ftp.c $
...
static char fullpass[FILENAME_MAX+1];
...
戻り値	 0: OK
		-1: ERR
----------------------------------------------------------------*/
static char GetFullPass( char *name )
{
	char pass[FILENAME_MAX+1], pass_len;

	if(	name[1] != ':' || name[2] != '\\' ) {
		memset( pass, 0, sizeof(pass) );
		getcwd( pass, sizeof(pass) );
		pass_len = strlen(pass);
		if( pass[pass_len-1]!='\\' ) {
			pass[pass_len]='\\';
			pass[pass_len+1]=0x00;
		}
		memmove( name+strlen(pass), name, strlen(name)+1 );
		memcpy( name, pass, strlen(pass) );
		return 0;
	}
	return -1;
}
...
static int      FTPtx(MESS * mp)
{
    ...
    *cp = 0;
	...
	GetFullPass( fullpass );
    ffp = fopen(fullpass, "rb");
...
#endif
    for (;;) {
        len = fread(bp, 1, SOCKET_MAXDAT(conno), ffp);
        if (len <= 0)
            break;
        l2 += len;
        if (l2 >= tlen)
...
/*    cp = mp->netno == 14 ? "w" : "a";*/
    ...
    for (;;) {
        ...
        i1 = fwrite((char *) mp + mp->offset, len, 1, ffp);
        Nrelbuf(mp);
        if (i1 <= 0) {
#if NTRACE >= 1
...
*/
    ...
    loggedin = herport = 0;
    SOCKET_PUSH(conno);
    Nwrite(conno, "220 DACS-I FTP server ready.\r\n", 30);
        ...
        case 27:                /* NLST */
            ...
            for (;;) {
				...
				if (cno == 27) {
                    ...
                    memset(rp, ' ', 80);
                    memcpy(rp, ffblk.ff_name, strlen(ffblk.ff_name));
                    ...
                    if (i1 > 13)
                        i1 -= 12;
                    Nsprintf(rp + 18, "%9ld %02d-%02d-%02d  %02d:%02d%c",
