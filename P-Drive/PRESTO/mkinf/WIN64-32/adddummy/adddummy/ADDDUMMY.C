...
#define		FLASH_WRITE_BLOCK   (0x02000)		/* for DMU */
#define     MAX_ITEM   5
#define     MCU1     1
#define     MCU2     2
...
mddladr:
		...
		for(j=10;i>0;--i){
			// a = htoi(buf[j++])*16+htoi(buf[j++]);
			// 上記の様に書くと、リリースビルド時に計算がずれる
			// インクリメントしながら足し算をすると、後ろの数字(下位の数字のインクリメントが効かず、上位と同じ数字が入る)
			a = htoi(buf[j++]) * 16;
			a += htoi(buf[j++]);

			sumtmp += (unsigned long)a;
			++sumcnt;
			p++;
			sum += a;
		}
...
//	return(size);
	...
	if((size % block)!=0) {
		// 書き込みの区切りデータ数に調整する
		block_cnt = ( size / block ) + 1;
	}
	else {
		block_cnt = size / block;
	}

	// 区切り領域の手前4バイトに0xFFを書くので、
	// 区切り領域ぎりぎりまでプログラム領域があった時でも
	// データをつぶさないように、データの使用領域の終端の
	// 次の次の区切り領域までプログラムを使用するようにする。
	return( ( block_cnt + 1 ) * block );
}
